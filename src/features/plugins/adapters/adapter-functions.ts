...
export function registerFunctionDecorator(
  functionName: string,
  plugin: CallbackDecoratorOptions
) {
...
export function withOptionalFunctionPlugins<
  ...
  F extends (...arguments_: any[]) => any
>(rootFunction: F, functionName: string) {
  return (...params: Parameters<F>) => {
    const plugins =
      pluginRegistryCallbacks.get(functionName)?.sort(sortByPriority) || [];
    ...
    for (const plugin of pluginsAfter) {
      const response = plugin.callback({
        params: paramsToUse,
        returnValue: returnValueToUse,
      });
